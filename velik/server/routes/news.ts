...
router.get("/news/by-slug/:slug", async (req, res): Promise<void> => {
  const [row] = await db.select().from(newsTable).where(eq(newsTable.slug, req.params.slug));
